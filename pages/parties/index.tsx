...
export default function Home({ page:initialPage }: HomeProps) {
  ...
  const { meta, party, parties, page, secret, edited, faction, loading, search } = state
  ...
  useEffect(() => {
    ...
    if (page !== initialPageNum) {
      dispatch({ type: PartiesActions.PAGE, payload: initialPageNum || 1 })
    }
  ...
  useEffect(() => {
    async function reload() {
      try {
        console.log("Fetching Parties page ", { page, initialPageNum })
        const data = await client.getParties(fetchPayload)
